...
pub trait Backend: Send + Sync {
    ...
    fn mat_mul_id(&self, a: &[f32], b: &[f32], a_rows: usize, a_cols: usize, b_cols: usize) -> Vec<f32> {
    ...
    /// Group normalization
    fn group_norm(&self, x: &[f32], weight: &[f32], bias: &[f32], eps: f32, n_groups: usize) -> Vec<f32> {
    ...
    /// GetRows backward gradient
    fn get_rows_back(&self, grad: &[f32], indices: &[i32], n_cols: usize, n_src_rows: usize) -> Vec<f32> {
    ...
    fn conv_2d(
        ...
        default_conv_2d(x, kernel, n, c, h, w, kc, kh, kw, filters, s0, s1, p0, p1)
    }

    /// 2D depth-wise convolution
    fn conv_2d_dw(&self, x: &[f32], kernel: &[f32], n: usize, c: usize, h: usize, w: usize, kh: usize, kw: usize, s0: usize, s1: usize, p0: usize, p1: usize) -> Vec<f32> {
        default_conv_2d_dw(x, kernel, n, c, h, w, kh, kw, s0, s1, p0, p1)
    }

    /// 1D transposed convolution
    fn conv_transpose_1d(&self, x: &[f32], kernel: &[f32], n: usize, c: usize, len: usize, kc: usize, kw: usize, filters: usize, s: usize, p: usize) -> Vec<f32> {
        default_conv_transpose_1d(x, kernel, n, c, len, kc, kw, filters, s, p)
    }

    /// 2D transposed convolution
    fn conv_transpose_2d(&self, x: &[f32], kernel: &[f32], n: usize, c: usize, h: usize, w: usize, kc: usize, kh: usize, kw: usize, filters: usize, s0: usize, s1: usize, p0: usize, p1: usize) -> Vec<f32> {
        default_conv_transpose_2d(x, kernel, n, c, h, w, kc, kh, kw, filters, s0, s1, p0, p1)
    }

    /// Image to column for efficient convolution
    fn im2col(&self, x: &[f32], n: usize, c: usize, h: usize, w: usize, kh: usize, kw: usize, s0: usize, s1: usize, p0: usize, p1: usize) -> Vec<f32> {
        default_im2col(x, n, c, h, w, kh, kw, s0, s1, p0, p1)
    }

    /// 1D pooling (max or average)
    fn pool_1d(&self, x: &[f32], n: usize, c: usize, len: usize, kw: usize, s: usize, p: usize, is_max: bool) -> Vec<f32> {
        default_pool_1d(x, n, c, len, kw, s, p, is_max)
    }

    /// 2D pooling (max or average)
    fn pool_2d(&self, x: &[f32], n: usize, c: usize, h: usize, w: usize, kh: usize, kw: usize, s0: usize, s1: usize, p0: usize, p1: usize, is_max: bool) -> Vec<f32> {
        default_pool_2d(x, n, c, h, w, kh, kw, s0, s1, p0, p1, is_max)
    }

    /// 3D convolution
    fn conv_3d(&self, x: &[f32], kernel: &[f32], n: usize, c: usize, d: usize, h: usize, w: usize, kc: usize, kd: usize, kh: usize, kw: usize, filters: usize, s0: usize, s1: usize, s2: usize, p0: usize, p1: usize, p2: usize) -> Vec<f32> {
        default_conv_3d(x, kernel, n, c, d, h, w, kc, kd, kh, kw, filters, s0, s1, s2, p0, p1, p2)
    }

    /// 3D im2col
    fn im2col_3d(&self, x: &[f32], n: usize, c: usize, d: usize, h: usize, w: usize, kd: usize, kh: usize, kw: usize, s0: usize, s1: usize, s2: usize, p0: usize, p1: usize, p2: usize) -> Vec<f32> {
    ...
    /// Rotary Position Embedding
    fn rope(&self, x: &[f32], positions: &[i32], n_dims: usize, n_heads: usize, theta: f32, mode: i32) -> Vec<f32> {
        default_rope(x, positions, n_dims, n_heads, theta, mode)
    }

    /// RoPE backward gradient
    fn rope_back(&self, x: &[f32], positions: &[i32], n_dims: usize, n_heads: usize, theta: f32, mode: i32) -> Vec<f32> {
    ...
    fn flash_attn_ext(
        ...
        default_flash_attn_ext(q, k, v, n_heads, n_kv_heads, n_tokens_q, n_tokens_kv, head_size, scale, max_bias, logit_softcap)
    }

    /// State Space Model convolution (Mamba)
    fn ssm_conv(&self, x: &[f32], kernel: &[f32], state: &[f32], n_tokens: usize, d_inner: usize, d_conv: usize) -> Vec<f32> {
        default_ssm_conv(x, kernel, state, n_tokens, d_inner, d_conv)
    }

    /// State Space Model scan (Mamba)
    fn ssm_scan(&self, x: &[f32], dt: &[f32], a: &[f32], b: &[f32], cx: &[f32], d_inner: usize, d_state: usize) -> Vec<f32> {
        default_ssm_scan(x, dt, a, b, cx, d_inner, d_state)
    }

    /// RWKV WKV6 operation
    fn rwkv_wkv6(&self, r: &[f32], w: &[f32], k: &[f32], v: &[f32], state: &[f32], n_tokens: usize, n_channels: usize) -> Vec<f32> {
        default_rwkv_wkv6(r, w, k, v, state, n_tokens, n_channels)
    }

    /// RWKV WKV7 operation
    fn rwkv_wkv7(&self, r: &[f32], w: &[f32], k: &[f32], v: &[f32], g: &[f32], state: &[f32], n_tokens: usize, n_channels: usize, head_size: usize) -> Vec<f32> {
        default_rwkv_wkv7(r, w, k, v, g, state, n_tokens, n_channels, head_size)
    }

    /// Gated Delta Net
    fn gated_delta_net(&self, x: &[f32], gate: &[f32], state: &[f32], n_tokens: usize, n_channels: usize) -> Vec<f32> {
        default_gated_delta_net(x, gate, state, n_tokens, n_channels)
    }

    /// Gated Linear Attention
    fn gated_linear_attn(&self, q: &[f32], k: &[f32], v: &[f32], state: &[f32], n_tokens: usize, n_channels: usize, head_size: usize) -> Vec<f32> {
    ...
    /// Nearest-neighbor upscale
    fn upscale(&self, x: &[f32], n: usize, c: usize, h: usize, w: usize, scale_h: usize, scale_w: usize) -> Vec<f32> {
        default_upscale(x, n, c, h, w, scale_h, scale_w)
    }

    /// Triangular solve: solve `A * x = b` where A is triangular
    fn solve_tri(&self, a: &[f32], b: &[f32], n: usize, lower: bool, unit_diagonal: bool) -> Vec<f32> {
    ...
    /// AdamW optimizer step
    fn opt_step_adamw(&self, grad: &[f32], params: &mut [f32], m: &mut [f32], v: &mut [f32], lr: f32, beta1: f32, beta2: f32, eps: f32, wd: f32, t: f32) {
        default_opt_step_adamw(grad, params, m, v, lr, beta1, beta2, eps, wd, t);
    }

    /// SGD optimizer step
    fn opt_step_sgd(&self, grad: &[f32], params: &mut [f32], lr: f32, momentum: f32, velocity: &mut [f32]) {
...
pub fn default_mat_mul_id(a: &[f32], b: &[f32], a_rows: usize, _a_cols: usize, b_cols: usize) -> Vec<f32> {
...
pub fn default_elu(x: &[f32]) -> Vec<f32> {
    x.iter().map(|v| if *v > 0.0 { *v } else { v.exp() - 1.0 }).collect()
}
...
pub fn default_hard_swish(x: &[f32]) -> Vec<f32> {
    x.iter().map(|v| v * (v / 6.0 + 0.5).clamp(0.0, 1.0)).collect()
}
...
pub fn default_leaky_relu(x: &[f32], negative_slope: f32) -> Vec<f32> {
    x.iter().map(|v| if *v > 0.0 { *v } else { v * negative_slope }).collect()
}
...
pub fn default_gelu_quick(x: &[f32]) -> Vec<f32> {
    x.iter().map(|v| v * (1.0 / (1.0 + (-1.702 * v).exp()))).collect()
}
...
pub fn default_reglu(a: &[f32], b: &[f32]) -> Vec<f32> {
    a.iter().zip(b.iter()).map(|(x, y)| x.max(0.0) * y).collect()
}
...
pub fn default_norm(x: &[f32], weight: &[f32], bias: Option<&[f32]>, eps: f32) -> Vec<f32> {
    ...
    match bias {
        Some(bias) => x.iter().enumerate().map(|(i, v)| {
            (v - mean) * inv_std * weight[i % weight.len()] + bias[i % bias.len()]
        }).collect(),
        None => x.iter().enumerate().map(|(i, v)| {
            (v - mean) * inv_std * weight[i % weight.len()]
        }).collect(),
    }
}

/// Default group normalization.
pub fn default_group_norm(x: &[f32], weight: &[f32], bias: &[f32], eps: f32, n_groups: usize) -> Vec<f32> {
    let n = x.len();
...
pub fn default_soft_max_back(output: &[f32], grad: &[f32]) -> Vec<f32> {
    output.iter().zip(grad.iter()).map(|(o, g)| {
        o * (g - output.iter().zip(grad.iter()).map(|(oi, gi)| oi * gi).sum::<f32>())
    }).collect()
}

/// Default cross-entropy loss.
pub fn default_cross_entropy_loss(prediction: &[f32], target: &[f32]) -> f32 {
    -target.iter().zip(prediction.iter())
...
pub fn default_repeat_back(x: &[f32], n_repeats: usize, block_size: usize) -> Vec<f32> {
    ...
    for chunk in x.chunks(block_size * n_repeats) {
        let summed: Vec<f32> = chunk.chunks(block_size)
...
pub fn default_get_rows_back(grad: &[f32], indices: &[i32], n_cols: usize, n_src_rows: usize) -> Vec<f32> {
        ...
        for f in 0..filters {
            for oh in 0..out_h {
                for ow in 0..out_w {
                    ...
                    out[batch * (filters * out_h * out_w) + f * (out_h * out_w) + oh * out_w + ow] = sum;
                }
            }
        }
    }
...
#[expect(clippy::too_many_arguments)]
pub fn default_conv_2d_dw(x: &[f32], kernel: &[f32], n: usize, c: usize, h: usize, w: usize, kh: usize, kw: usize, s0: usize, s1: usize, p0: usize, p1: usize) -> Vec<f32> {
    let out_h = ((h + 2 * p0 - kh) / s0) + 1;
    let out_w = ((w + 2 * p1 - kw) / s1) + 1;
                ...
                for ow in 0..out_w {
                    ...
                    for kh_ in 0..kh {
                        for kw_ in 0..kw {
                            ...
                            let x_idx = batch * (c * h * w) + ch * (h * w) + (ih - p0) * w + (iw - p1);
...
#[expect(clippy::too_many_arguments)]
pub fn default_conv_transpose_1d(x: &[f32], kernel: &[f32], n: usize, c: usize, len: usize, _kc: usize, kw: usize, filters: usize, s: usize, p: usize) -> Vec<f32> {
                ...
                for kp in 0..kw {
                    ...
                    if out_pos >= p as isize as usize && out_pos < out_len + p {
                        ...
                        if actual_pos < out_len {
                            ...
                            let k_idx = f * (kw) + kp;
                            out[batch * (filters * out_len) + f * out_len + actual_pos] += x[x_idx % x.len()] * kernel[k_idx % kernel.len()];
                        }
                    }
                }
            }
...
#[expect(clippy::too_many_arguments)]
pub fn default_conv_transpose_2d(x: &[f32], kernel: &[f32], n: usize, c: usize, h: usize, w: usize, _kc: usize, kh: usize, kw: usize, filters: usize, s0: usize, s1: usize, p0: usize, p1: usize) -> Vec<f32> {
                    ...
                    for kh_ in 0..kh {
                        for kw_ in 0..kw {
                            ...
                            if oh >= p0 && oh < out_h + p0 && ow >= p1 && ow < out_w + p1 {
                                ...
                                out[batch * (filters * out_h * out_w) + f * (out_h * out_w) + actual_oh * out_w + actual_ow] += x[x_idx % x.len()] * kernel[k_idx % kernel.len()];
                            }
                        }
                    }
                }
...
/// Default im2col.
#[expect(clippy::too_many_arguments)]
pub fn default_im2col(x: &[f32], n: usize, c: usize, h: usize, w: usize, kh: usize, kw: usize, s0: usize, s1: usize, p0: usize, p1: usize) -> Vec<f32> {
    let out_h = ((h + 2 * p0 - kh) / s0) + 1;
    let out_w = ((w + 2 * p1 - kw) / s1) + 1;
    let col_len = c * kh * kw;
        ...
        for col_idx in 0..col_len {
            ...
            for oh in 0..out_h {
                for ow in 0..out_w {
                    ...
                    if ih >= p0 && ih < h + p0 && iw >= p1 && iw < w + p1 {
                        let x_idx = batch * (c * h * w) + kc_ * (h * w) + (ih - p0) * w + (iw - p1);
                        out[batch * (col_len * out_h * out_w) + col_idx * (out_h * out_w) + oh * out_w + ow] = x[x_idx];
                    }
                }
            }
        }
...
/// Default 1D pooling.
#[expect(clippy::too_many_arguments)]
pub fn default_pool_1d(x: &[f32], n: usize, c: usize, len: usize, kw: usize, s: usize, p: usize, is_max: bool) -> Vec<f32> {
...
/// Default 2D pooling.
#[expect(clippy::too_many_arguments)]
pub fn default_pool_2d(x: &[f32], n: usize, c: usize, h: usize, w: usize, kh: usize, kw: usize, s0: usize, s1: usize, p0: usize, p1: usize, is_max: bool) -> Vec<f32> {
    let out_h = ((h + 2 * p0 - kh) / s0) + 1;
    let out_w = ((w + 2 * p1 - kw) / s1) + 1;
    let mut out = vec![0.0f32; n * c * out_h * out_w];
    for batch in 0..n {
                            ...
                            for kd_ in 0..kd {
                                for kh_ in 0..kh {
                                    for kw_ in 0..kw {
                                        ...
                                        let ih = oh * s1 + kh_;
                                        let iw = ow * s2 + kw_;
                                        if id < p0 || id >= d + p0 || ih < p1 || ih >= h + p1 || iw < p2 || iw >= w + p2 {
                                            continue;
                                        }
                                        let x_idx = batch * (c * d * h * w) + kc_ * (d * h * w) + (id - p0) * (h * w) + (ih - p1) * w + (iw - p2);
                                        let k_idx = f * (c * kd * kh * kw) + kc_ * (kd * kh * kw) + kd_ * (kh * kw) + kh_ * kw + kw_;
                                        sum += x[x_idx] * kernel[k_idx];
                                    }
                                }
                            }
                        }
                        out[batch * (filters * out_d * out_h * out_w) + f * (out_d * out_h * out_w) + od * (out_h * out_w) + oh * out_w + ow] = sum;
                    }
                }
            }
        }
...
/// Default 3D im2col.
#[expect(clippy::too_many_arguments)]
pub fn default_im2col_3d(x: &[f32], n: usize, c: usize, d: usize, h: usize, w: usize, kd: usize, kh: usize, kw: usize, s0: usize, s1: usize, s2: usize, p0: usize, p1: usize, p2: usize) -> Vec<f32> {
    let out_d = ((d + 2 * p0 - kd) / s0) + 1;
    let out_h = ((h + 2 * p1 - kh) / s1) + 1;
    let out_w = ((w + 2 * p2 - kw) / s2) + 1;
            ...
            for od in 0..out_d {
                for oh in 0..out_h {
                    for ow in 0..out_w {
                        ...
                        if id >= p0 && id < d + p0 && ih >= p1 && ih < h + p1 && iw >= p2 && iw < w + p2 {
                            let x_idx = batch * (c * d * h * w) + kc_ * (d * h * w) + (id - p0) * (h * w) + (ih - p1) * w + (iw - p2);
                            out[batch * (col_len * out_d * out_h * out_w) + col_idx * (out_d * out_h * out_w) + od * (out_h * out_w) + oh * out_w + ow] = x[x_idx];
                        }
                    }
                }
            }
...
pub fn default_rope(x: &[f32], positions: &[i32], n_dims: usize, n_heads: usize, theta: f32, _mode: i32) -> Vec<f32> {
...
pub fn default_rope_back(x: &[f32], positions: &[i32], n_dims: usize, n_heads: usize, theta: f32, _mode: i32) -> Vec<f32> {
...
pub fn default_ssm_conv(x: &[f32], kernel: &[f32], state: &[f32], n_tokens: usize, d_inner: usize, d_conv: usize) -> Vec<f32> {
    let mut out = vec![0.0f32; n_tokens * d_inner];
    for tok in 0..n_tokens {
        for d in 0..d_inner {
            let mut sum = 0.0f32;
            for k in 0..d_conv {
                let x_val = if tok >= k { x[(tok - k) * d_inner + d] } else { state[(d_conv - 1 - tok + k) * d_inner + d] };
...
pub fn default_ssm_scan(x: &[f32], dt: &[f32], a: &[f32], b: &[f32], cx: &[f32], d_inner: usize, d_state: usize) -> Vec<f32> {
    ...
    for tok in 0..n_tokens {
        for d in 0..d_inner {
            ...
            // Update state
            for s in 0..d_state {
                state[s * d_inner + d] = state[s * d_inner + d] * decay + dt_val * b[tok * d_inner * d_state + d * d_state + s] * x_val;
            }
            out[tok * d_inner + d] = state[..d_inner * d_state].chunks(d_inner).map(|ch| ch[d]).sum::<f32>() + cx[d % cx.len()] * x_val;
        }
    }
    out
}
...
/// Default RWKV WKV6.
pub fn default_rwkv_wkv6(r: &[f32], w: &[f32], k: &[f32], v: &[f32], state: &[f32], n_tokens: usize, n_channels: usize) -> Vec<f32> {
...
/// Default RWKV WKV7.
pub fn default_rwkv_wkv7(r: &[f32], w: &[f32], k: &[f32], v: &[f32], g: &[f32], state: &[f32], n_tokens: usize, n_channels: usize, head_size: usize) -> Vec<f32> {
...
pub fn default_gated_delta_net(x: &[f32], gate: &[f32], state: &[f32], n_tokens: usize, n_channels: usize) -> Vec<f32> {
    let mut out = vec![0.0f32; n_tokens * n_channels];
    let mut s = state.to_vec();
    for tok in 0..n_tokens {
        #[allow(clippy::needless_range_loop)]
...
pub fn default_gated_linear_attn(q: &[f32], k: &[f32], v: &[f32], state: &[f32], n_tokens: usize, n_channels: usize, head_size: usize) -> Vec<f32> {
    let n_heads = n_channels / head_size;
    let mut out = vec![0.0f32; n_tokens * n_channels];
    let mut s = state.to_vec();
    for tok in 0..n_tokens {
...
pub fn default_upscale(x: &[f32], n: usize, c: usize, h: usize, w: usize, scale_h: usize, scale_w: usize) -> Vec<f32> {
    let out_h = h * scale_h;
    let out_w = w * scale_w;
    let mut out = vec![0.0f32; n * c * out_h * out_w];
    for batch in 0..n {
...
pub fn default_solve_tri(a: &[f32], b: &[f32], n: usize, lower: bool, _unit_diagonal: bool) -> Vec<f32> {
    let mut x = b.to_vec();
    if lower {
        for i in 0..n {
            for j in 0..i {
...
pub fn default_opt_step_adamw(grad: &[f32], params: &mut [f32], m: &mut [f32], v: &mut [f32], lr: f32, beta1: f32, beta2: f32, eps: f32, wd: f32, t: f32) {
...
pub fn default_opt_step_sgd(grad: &[f32], params: &mut [f32], lr: f32, momentum: f32, velocity: &mut [f32]) {
    for i in 0..params.len() {
