...
/// Extension trait providing de‑quantization for [`TensorInfo`].
///
/// This trait is defined here to avoid the orphan rule, allowing us to add a
/// `dequantize` method that returns a `Vec<f32>` from the raw tensor bytes.
pub trait TensorInfoExt {
    /// De‑quantize the raw tensor bytes into a vector of `f32` values.
    ///
    /// Returns a [`GgufError`] if the data cannot be decoded.
...
// Re-export the extension trait for external use. (Removed to avoid duplicate definition error.)
// Wrapper around `TensorInfo` to allow implementing methods without orphan rule.
/// Wrapper around [`TensorInfo`] that forwards de‑quantization calls.
#[derive(Debug, Clone)]
pub struct TensorInfoWrapper(pub TensorInfo);

impl TensorInfoWrapper {
    /// De‑quantize the raw tensor bytes into a `Vec<f32>`.
    /// Delegates to the inner `TensorInfo` implementation.
    pub fn dequantize(&self, raw: &[u8]) -> Result<Vec<f32>, GgufError> {
        TensorInfoExt::dequantize(&self.0, raw)
    }
}
pub use tensor::{mmap_tensor_dequantize, TensorDequantExt};
