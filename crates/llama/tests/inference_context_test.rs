...
fn create_test_gguf(path: &std::path::Path) -> std::io::Result<()> {
    ...
    let write_string = |f: &mut std::fs::File, s: &str| -> std::io::Result<()> {
    ...
    let write_kv_u32 = |f: &mut std::fs::File, key: &str, v: u32| -> std::io::Result<()> {
        write_string(f, key)?;
    ...
    let write_kv_str = |f: &mut std::fs::File, key: &str, v: &str| -> std::io::Result<()> {
        write_string(f, key)?;
    ...
    // Tensor info
    let write_tensor_info =
        |f: &mut std::fs::File, name: &str, shape: &[i64], dtype: i32| -> std::io::Result<()> {
            write_string(f, name)?;
    ...
    // Tensor data (all F32)
    let write_tensor = |f: &mut std::fs::File, data: &[f32]| -> std::io::Result<()> {
        for &v in data {
            f.write_all(&v.to_le_bytes())?;
        }
        Ok(())
...
fn test_load_real_gguf_model() {
    ...
    if !model_path.exists() {
        println!(
            "Skipping test_load_real_gguf_model: test model not found at {:?}",
            model_path
        );
        return;
    }
